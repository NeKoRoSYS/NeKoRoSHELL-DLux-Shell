...
Item {
    ...
    component PillDelegate: Loader {
        ...
        Binding { when: status === Loader.Ready && item !== null && ("inPill" in item); target: item; property: "inPill"; value: true }
        Binding { when: status === Loader.Ready && item !== null && ("barScreen" in item); target: item; property: "barScreen"; value: root.barScreen }
    }
